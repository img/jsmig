...
(function () {
    ...
    com.ibm.rdm.web.server.client.RDMClient = {
        ...
        CACHE_TYPE_UPDATECONTEXT: "UpdateContext",
        CACHE_TYPE_IFNONEMATCH_ETAG: "IfNoneMatch",
        ///////////////////////////////////////////////////////////////////////////////////
        // API
        doGet: function (args) {
            /*args: {
        ...
        doMultiGet: function (reqArgsArray, /*Integer?*/ chunkSize, /*Boolean?*/ skipCache, /*String?*/ componentURI, /*optional*/ xhrArgs) {
            //var xhrArgs = {};
        ...
        doMultiRequest: function (reqArgsArray, /*Integer?*/ chunkSize, /*Boolean?*/ skipCache, /*String?*/ componentURI, /*Object?*/ xhrArgs) {
            // summary:
            ...
            var makeChunkRequest = dojo.hitch(this, function () {
                ...
                var multiReqPostData = this._assembleMultiRequestFragment(chunkReqArgs);
            ...
            handlePage = dojo.hitch(this, function (multiReqResponses) {
                var parsedResponses = this._parseMultiRequestResponse(multiReqResponses);
        ...
        _parseMultiRequestResponse: function (multiReqResponse) {
            var multiReponseNodes = domutils.getElementsByTagNameNS(multiReqResponse.doc, "MultiResponse", MREQ_NAMESPACE);
            var parsedResponses = dojo.map(multiReponseNodes, function (multiReponseNode) {
                var response = { headers: {} };
                var statusCodeNode = domutils.getSingleElementByTagNameNS(multiReponseNode, "statusCode", MREQ_NAMESPACE);
                var reasonPhraseNode = domutils.getSingleElementByTagNameNS(multiReponseNode, "reasonPhrase", MREQ_NAMESPACE);
                var httpHeaderNodes = domutils.getElementsByTagNameNS(multiReponseNode, "httpHeader", MREQ_NAMESPACE);
                var responseContentNode = domutils.getSingleElementByTagNameNS(multiReponseNode, "responseContent", MREQ_NAMESPACE);
                response.url = domutils.getAttributeNS(multiReponseNode, "about", RDF_NAMESPACE);
                response.statusCode =
                ...
                dojo.forEach(httpHeaderNodes, function (headerNode) {
                    var nameNode = domutils.getSingleElementByTagNameNS(headerNode, "httpHeaderName", MREQ_NAMESPACE);
                    var valueNode = domutils.getSingleElementByTagNameNS(headerNode, "httpHeaderValue", MREQ_NAMESPACE);
    ...
    com.ibm.rdm.web.server.client._RMBulkSave = {
        ...
        _processBatchResponse: function (argsMap, multiResponse) {
            ...
            LockManager.releaseLockedResources(locks).then(dojo.hitch(this, function () {
                dojo.forEach(multiResponse, function (response) {
                    var statusCode = response.statusCode, reqArray = argsMap[response.url], deferred = reqArray[1];
